...
def render_metric_grid(findings) -> None:
    ...
    first_row[3].metric("Bridge status", findings.bridge_status)

    second_row = st.columns(4)
    second_row[0].metric("Materiality", money(findings.materiality))
    second_row[1].metric("Total reconciling items", money(findings.total_reconciling_items))
    second_row[2].metric("RCM context", money(findings.reverse_charge_context))
    second_row[3].metric("Pre-filing exception score", f"{findings.score.score} - {findings.score.label}")
...
st.set_page_config(page_title="UAE VAT-to-CT Bridge", layout="wide")
st.markdown(
    """
    <style>
...
demo_col, template_col = st.columns([1, 2])
with demo_col:
...
with dashboard_tab:
    if not inputs or not findings:
        ...
        driver_col, check_col = st.columns([1, 1])
        with driver_col:
...
with exports_tab:
    if not inputs or not findings:
        ...
        preview_col, pdf_col = st.columns([1, 1])
        with preview_col:
            ...
            st.caption(
                "Workbook sheets: Summary, VAT Returns, P&L Summary, CT Computation, "
                "Reconciling Items, Checks, Methodology"
            )
        with pdf_col:
